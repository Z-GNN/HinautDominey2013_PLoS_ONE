...
def simulation(root_file_name=None, N=100, sr=1, tau=6, act_time=20, subset=range(15,41),
               in_scal=0.75, ridge=10**-9, n_folds=None, seed=None,
...
#                internal_states_test[idx_out] = f_copy.inspect(reservoir)
        ...
        if comp_err:
            ...
            result_comp_err = compute_error(error_obj=err_obj, _states_out=states_out_test,
                                            ...
                                            verbose=False)
            (mean_meaning_err, std_meaning_err, median_meaning_err, mean_sent_err, std_sent_err, median_sent_err) = result_comp_err
        else:
            (mean_meaning_err, std_meaning_err, median_meaning_err, mean_sent_err, std_sent_err, median_sent_err) = (-1,-1,-1,-1,-1,-1)
        ...
        if plot_output:
            plot_tools.plot_output(forced_subset=subset, _outputs=states_out_test, d_io=d_io,
                ...
                subtitle=subtitle+"__m-err"+str(mean_meaning_err)+"_s-err"+str(mean_sent_err), window=0,
...
#                                   title="_internal_states", subtitle="", legend_=None)
    ...
    if verbose:
        ...
        print "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*--*"
    return (mdp.numx.mean(all_mean_meaning_err), mdp.numx.std(all_mean_meaning_err), mdp.numx.mean(all_mean_sent_err), mdp.numx.std(all_mean_sent_err))
...
#        curr_time = int(time.time()*10**6)
    ...
    return (mdp.numx.mean(list_mean_merr), mdp.numx.std(list_mean_merr), mdp.numx.mean(list_mean_serr), mdp.numx.std(list_mean_serr))
...
#    subset = range(15,45) # Exp 2 part 1
    ## Special set 15 to 41 without sentences with "that was": 24 25 26 27 32 33 38 39 (i.e. 9-10-11-12-17-18-23-24 starting from 15)
...
#        comp_err=True, plot_output=True, comp_diff_states=True, verbose=False)
    (mean_me, std_me, mean_se, std_se) = multi_sim(nr_instance=1,
        root_file_name='../RES_TEMP/run_multi_simple_xp',
        N=300, sr=1, tau=6, act_time=20, subset=subset,
        in_scal=0.75, ridge=10**-9, n_folds=0, seed=1,
        comp_err=True, plot_output=True, comp_diff_states=True, verbose=False)
    print (mean_me, std_me, mean_se, std_se)
    
#  Parameter valeus    
#            N  act_time   n_folds  plot_output
# Exp1     300   20         0         True
# Exp2.1   300   20         0         True
# Exp2.2   300   20         0         True  (change corpus as described above)
# Exp3     100    1         -1        True - does cross validation
#    change seed=2
# Exp3     100    1         -1        False - does cross validation and runs faster
# Exp3  keep these parameter & set nr_instance=20 to average over 20 instances
# Exp4     300   20         0         True
